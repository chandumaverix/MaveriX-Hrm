...
const CATEGORIES = [
  'all','auth','employee','leave','payroll','role',
...
export default function ActivityPage() {
  ...
  const fetchLogs = useCallback(async (pageNum: number, isReset: boolean) => {
    setLoading(true)
    let q = supabase
    ...
    setLoading(false)
  }, [role, cat, search, supabase])

  useEffect(() => { 
    setPage(0)
    fetchLogs(0, true) 
  }, [role, cat, search, fetchLogs])

  // Real-time
  useEffect(() => {
    const ch = supabase.channel('hrm_activity')
    ...
    <div className="max-w-5xl mx-auto p-6 space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Every action across your HRM in real time</p>
        </div>
      ...
      <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-2">
        {Object.entries(CAT_META).map(([k, v]) => (
          <button key={k} onClick={() => setCat(cat === k ? 'all' : k)}
            className={`rounded-xl p-2.5 text-center border transition-all text-left ${
              cat === k ? 'border-gray-400 ring-1 ring-gray-300' : 'border-gray-100 hover:border-gray-200'
            } bg-white`}>
            <div className={`text-xs font-medium ${v.color} px-1.5 py-0.5 rounded-md inline-block mb-1`}>
              {v.label}
            </div>
            <div className="text-lg font-semibold text-gray-900">{counts[k] || 0}</div>
          </button>
        ))}
      </div>
      ...
      <div className="space-y-1.5">
        {loading && !logs.length ? (
          ...
          <div className="text-center py-16 text-gray-400 text-sm">No activities found</div>
        ) : logs.map(log => {
          ...
          return (
            <div key={log.id}
              ...
              <div className={`w-8 h-8 rounded-full flex-shrink-0 flex items-center justify-center text-xs font-bold ${c?.color}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
                  ...
                  <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium capitalize ${ROLE_COLOR[log.user_role]}`}>
                    {log.user_role}
                  </span>
                  <span className={`text-xs px-1.5 py-0.5 rounded-full font-medium ${c?.color}`}>
                    {c?.label}
                  </span>
                </div>
              ...
              <span className="text-xs text-gray-400 flex-shrink-0 pt-0.5">{timeAgo(log.created_at)}</span>
            </div>
          )
        })}
      </div>
